 ...
 */
#define list_for_each(pos, head) for (pos = (head)->next, __builtin_prefetch(pos->next,0,1); pos != (head); pos = pos->next, __builtin_prefetch(pos->next,0,1))
 ...
 * @head:	the head for your list.
 */
#define list_for_each_prev(pos, head) for (pos = (head)->prev, __builtin_prefetch(pos->prev,0,1); pos != (head); pos = pos->prev, __builtin_prefetch(pos->prev,0,1))

#define __list_for_each_prev(pos, head) for (pos = (head)->prev; pos != (head); pos = pos->prev)
 ...
 * @member:	the name of the list_struct within the struct.
 */
#define list_for_each_entry(pos, head, member)				for (pos = list_entry((head)->next, typeof(*pos), member),__builtin_prefetch(pos->member.next,0,1);&pos->member != (head); pos = list_entry(pos->member.next, typeof(*pos), member),__builtin_prefetch(pos->member.next,0,1))
